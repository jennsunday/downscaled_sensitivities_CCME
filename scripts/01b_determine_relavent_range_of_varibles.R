...
present_pH_surf_12km[!big_shelf_mask==1] <- NA
future_pH_surf_12km[!big_shelf_mask==1] <- NA
present_pH_bot_12km[!big_shelf_mask==1] <- NA
future_pH_bot_12km[!big_shelf_mask==1] <- NA
...
#pH
present_pH_surf_12km.df<-melt(present_pH_surf_12km) %>%
  mutate(lat=rep(1:dim(present_pH_surf_12km)[1], dim(present_pH_surf_12km)[2])) %>%
  separate(variable, c(NA, "long"), sep = "X", remove = TRUE) %>%
  mutate(long=as.numeric(long))%>%
  mutate(lat=as.numeric(lat))%>%
  mutate(latlong=paste(long, lat, sep="_"))

future_pH_surf_12km.df<-melt(future_pH_surf_12km) %>%
  mutate(lat=rep(1:dim(future_pH_surf_12km)[1], dim(future_pH_surf_12km)[2])) %>%
  separate(variable, c(NA, "long"), sep = "X", remove = TRUE) %>%
  mutate(long=as.numeric(long))%>%
  mutate(lat=as.numeric(lat))%>%
  mutate(latlong=paste(long, lat, sep="_"))

present_pH_bot_12km.df<-melt(present_pH_bot_12km) %>%
  mutate(lat=rep(1:dim(present_pH_bot_12km)[1], dim(present_pH_bot_12km)[2])) %>%
  separate(variable, c(NA, "long"), sep = "X", remove = TRUE) %>%
  mutate(long=as.numeric(long))%>%
  mutate(lat=as.numeric(lat))%>%
  mutate(latlong=paste(long, lat, sep="_"))

future_pH_bot_12km.df<-melt(future_pH_bot_12km) %>%
  mutate(lat=rep(1:dim(future_pH_bot_12km)[1], dim(future_pH_bot_12km)[2])) %>%
  separate(variable, c(NA, "long"), sep = "X", remove = TRUE) %>%
  mutate(long=as.numeric(long))%>%
  mutate(lat=as.numeric(lat))%>%
  mutate(latlong=paste(long, lat, sep="_"))
...
max_pH<-max(combined_pH$value, na.rm=T)
min_pH<-min(combined_pH$value, na.rm=T)
