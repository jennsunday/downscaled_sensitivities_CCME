...
join_grid_and_temp<-left_join(all_temp_responses, littletempbot.df)
join_response_and_temp<-left_join(test,temp_sens)
head(join_response_and_temp)

#make new column with % change
join_response_and_temp<-join_response_and_temp%>%
  mutate(percentchangetemp=value*weighted_sensitivity*100)

#now ask group by lat_long, filter by changes >10%, and ask the n() of each group size
little_temp_greater_than_10percent<-join_response_and_temp %>%
  separate(unique_response, sep="_and_", into=c("species", "response")) %>%
  filter(abs(percentchangetemp)>10) %>%
  group_by(latlong, species) %>%
  summarize(number_over_10=n()) %>%
  separate(latlong, sep="_", into=c("lat", "long")) %>%
  mutate(lat=as.numeric(lat), long=as.numeric(long))


little_temp_greater_than_10percent %>% 
  #filter(species=="sablefish") %>%
  ggplot(aes(x = lat, y = long)) + geom_tile(aes(fill = as.factor(number_over_10))) + 
  scale_fill_brewer(palette = "Reds") +
  facet_wrap(~species, nrow=2)
ggsave("figures/little_temp_greater_than_10percent.pdf", height = 6, width = 9)

write_csv(little_temp_greater_than_10percent, "processed_data/little_temp_greater_than_10percent.csv")
