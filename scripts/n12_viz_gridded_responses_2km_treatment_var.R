...
treatment_grid<-left_join(sensitivity_by_study, all_deltas_2km, by=c("treatment_var", "modelzone")) %>%
  drop_na(latlong) %>%
  drop_na(delta) %>%
  ...
  group_by(lat, long, pos_neg, treatment_var2) %>%
...
treatment_shelf<-left_join(treatment_grid, shelf_contour_2km, by=(c("lat", "long"))) %>%
  filter(shelf!=0)

unique(treatment_shelf$treatment_var2)


treatment_shelf %>% 
  ggplot(aes(y = lat, x = long)) + geom_tile(aes(fill = abs(weighted_sensitivity))) +
  geom_tile(data=coastline_mask, fill=grey(0.4)) +
  facet_grid(pos_neg~treatment_var2) + 
...
pos_neg_spread<-treatment_shelf %>%
  drop_na(weighted_sensitivity) %>%
  select(lat, long, pos_neg, weighted_sensitivity, treatment_var2) %>%
...
map<-data %>%
  ...
  theme_classic()+
  facet_wrap(~treatment_var2, nrow=1) +
