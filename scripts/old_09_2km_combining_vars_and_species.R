...
oxy200_greater_than_10percent<-read_csv("processed_data/oxy200_greater_than_10percent.csv")
oxybot_greater_than_10percent<-read_csv("processed_data/oxybot_greater_than_10percent.csv")
CO2200_greater_than_10percent<-read_csv("processed_data/CO2200_greater_than_10percent.csv")
CO2bot_greater_than_10percent<-read_csv("processed_data/CO2bot_greater_than_10percent.csv")
temp200_greater_than_10percent<-read_csv("processed_data/temp200_greater_than_10percent.csv")
tempbot_greater_than_10percent<-read_csv("processed_data/tempbot_greater_than_10percent.csv")
...
all_vars_together_200<-rbind(mutate(oxy200_greater_than_10percent, variable="oxy"),
                  ...
                  mutate(all_var_number_over_10=as.numeric(ifelse(
                    no_env_data=="no_data", "NA", all_var_number_over_10)))
...
all_vars_together_bot %>% 
  ...
  facet_wrap(~species2, nrow=2) +
  geom_text(data=num_studs, aes(x=165, y=15, label=all_var_num_responses))
ggsave("figures/all_vars_together_bot_10percent_dis.pdf", height = 6, width = 9)
...
all_vars_together_200 %>% 
  ...
  labs(fill="proportion of responses 
       studied altered by >10%")
ggsave("figures/all_vars_together_200_10percent_dis.pdf", height = 6, width = 9)
...
all_vars_together_relevant_layer<-rbind(all_vars_together_bot %>%
                                          filter(species %in% c("seagrass", 
                                                                ...
                                                                "Redurchin",
                                                                "RazorClam",
                                                                "copperquillbackrockfish","sablefish",  
                                                                "blueblackrockfish")) %>%
                                          mutate(layer="bottom"),
                                        all_vars_together_200 %>%
                                          filter(species %in% c("PinkSalmon")) %>%
                                          mutate(layer="200"))

#reorder
all_vars_together_relevant_layer<-all_vars_together_relevant_layer %>%
  mutate(category =  factor(species2, levels = order_desired)) %>%
  arrange(category)  

all_vars_together_relevant_layer$species2 <- factor(all_vars_together_relevant_layer$species2, 
                                         levels = rev(order_desired))


colourCount = length(unique(as.factor(all_vars_together_relevant_layer$all_var_number_over_10)))
all_vars_together_relevant_layer %>% 
  ggplot(aes(x = lat, y = long)) + geom_tile(aes(fill = as.factor(all_var_number_over_10))) + 
  facet_wrap(~species2+layer, nrow=2) +
  scale_fill_manual(values = colorRampPalette(brewer.pal(9, "YlOrRd"))(colourCount), na.value="grey") +
  #geom_text(data=num_studs, aes(x=165, y=15, label=all_var_num_responses)) +
  labs(fill="number of responses 
       studied altered by >10%") +
  theme_classic()
ggsave("figures/all_vars_together_rel_10percent_dis.pdf", height = 6, width = 9)

names(all_vars_together_relevant_layer)
#
